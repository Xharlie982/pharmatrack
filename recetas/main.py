...
app = FastAPI(
    title="Recetas y Dispensaciones API",
    version="1.2.0",
    description="Microservicio para la gestión de recetas médicas y su dispensación.",
...
def validar_producto(id_producto: str):
    ...
    try:
        ...
        if not data.get("activo"):
            raise HTTPException(status_code=409, detail=f"Operación rechazada: El producto '{id_producto}' está inactivo.")
            
    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            raise HTTPException(status_code=404, detail=f"Producto '{id_producto}' no encontrado en Catálogo.") from e
        raise HTTPException(status_code=503, detail=f"Error inesperado de Catálogo: {e.response.status_code}") from e
    except httpx.RequestError as e:
        if FAIL_CLOSED:
            raise HTTPException(status_code=503, detail=f"Servicio de Catálogo no disponible: {e}") from e
...
def validar_sucursal(id_sucursal: int):
    ...
    try:
        ...
        if e.response.status_code == 404:
            raise HTTPException(status_code=404, detail=f"Sucursal '{id_sucursal}' no encontrada en Inventario.") from e
        raise HTTPException(status_code=503, detail=f"Error inesperado de Inventario: {e.response.status_code}") from e
    except httpx.RequestError as e:
        if FAIL_CLOSED:
            raise HTTPException(status_code=503, detail=f"Servicio de Inventario no disponible: {e}") from e
...
def root():
    return {
        "service": "recetas",
        "message": "Bienvenido al microservicio de Recetas",
        "docs": f"{BASE_PATH}/docs",
        "health": f"{BASE_PATH}/healthz"
    }
