...
app.use(cors({
  ...
  credentials: true,
  allowedHeaders: ["Content-Type","Authorization","X-Correlation-Id","Idempotency-Key","X-Idempotencia"],
...
r.get("/disponibilidad", async (req,res)=>{
  ...
  try{
    ...
    let productos = [];
    if (producto){
      const p = await http.get(`${CATALOGO_URL}/productos/${encodeURIComponent(producto)}`, { headers:{ "X-Correlation-Id": c } }).then(r=>r.data);
      productos = p ? [p] : [];
    } else if (busqueda){
      // si tu catálogo aún no admite ?q, simplemente regresará []
      productos = await http.get(`${CATALOGO_URL}/productos`, { headers:{ "X-Correlation-Id": c }, params:{ q: busqueda } }).then(r=>r.data || []);
    } else {
      return res.status(400).json({code:"VALIDATION_ERROR", message:"producto o busqueda requerido"});
    }
    ...
    for (const prod of productos){
      ...
      const sucursales = (st||[]).filter(s => (s.stock_actual ?? s.cantidad_actual ?? 0) >= stock_minimo);
      items.push({ producto: prod, sucursales });
    }
    res.json({items});
  }catch(e){
    res.status(502).json({code:"DOWNSTREAM_ERROR", message:"Fallo consultando disponibilidad", details:String(e)});
  }
});
...
r.get("/ficha-producto/:producto", async (req,res)=>{
  ...
  try{
    ...
    const st = await http.get(`${INVENTARIO_URL}/stock`, { headers:{ "X-Correlation-Id": c }, params:{ id_producto: idp } }).then(r=>r.data || []);
    const porDistrito = {};
    for (const s of st){
    ...
    res.status(502).json({code:"DOWNSTREAM_ERROR", message:"Fallo consultando ficha", details:String(e)});
  }
});
...
r.post("/receta/validar", async (req,res)=>{
  ...
  try{
    ...
    res.status(todos?200:207).json({ id_receta:id, items, estado_sugerido: todos? "VALIDADA" : (items.some(i=>i.disponible>0) ? "PARCIAL" : "RECHAZADA") });
  }catch(e){
    res.status(502).json({code:"DOWNSTREAM_ERROR", message:"Fallo validando receta", details:String(e)});
  }
});
...
r.get("/receta/:id/validacion", async (req,res)=>{
  ...
  try{
    ...
    res.json({ id_receta: receta.id_receta, estado_sugerido: ok? "VALIDADA":(items.some(i=>i.disponible>0)?"PARCIAL":"RECHAZADA"), items });
  }catch(e){
    res.status(502).json({code:"DOWNSTREAM_ERROR", message:"Fallo consultando receta/stock", details:String(e)});
  }
});
...
r.post("/reserva-stock", async (req,res)=>{
  ...
  const idem = req.header("Idempotency-Key") || req.header("X-Idempotencia") || req.body?.idempotencia_clave || req.body?.idempotency_key;
  try{
    const id = req.body?.id_receta;
    if (!id) return res.status(400).json({code:"VALIDATION_ERROR", message:"id_receta requerido"});
    ...
    res.status(502).json({code:"DOWNSTREAM_ERROR", message:"Fallo al reservar", details:String(e)});
  }
});
