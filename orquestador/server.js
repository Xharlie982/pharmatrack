// server.js — Orquestador minimal (sin sugerencias ni reserva)
...
// Upstreams (usa DNS del ALB si así lo configuras en compose/env)
...
/**
 * GET /disponibilidad
 * Query: producto | busqueda (q), distrito (requerido), stock_minimo (def 1)
 * Devuelve productos + sucursales con stock >= stock_minimo en el distrito dado.
 */
r.get("/disponibilidad", async (req, res) => {
  ...
  try {
    ...
    const stock_minimo = Number(
      req.query.stock_minimo ?? req.query.minStock ?? 1
    );
    ...
    // Resolver productos desde Catálogo
    let productos = [];
    if (producto) {
      const p = await http
        .get(`${CATALOGO_URL}/productos/${encodeURIComponent(producto)}`, {
          headers: { "X-Correlation-Id": c },
        })
        .then((r) => r.data);
      productos = p ? [p] : [];
    } else if (busqueda) {
      // Si el catálogo aún no soporta ?q, devolverá [] y listo
      productos = await http
        .get(`${CATALOGO_URL}/productos`, {
          headers: { "X-Correlation-Id": c },
          params: { q: busqueda },
        })
        .then((r) => r.data || []);
...
/**
 * GET /ficha-producto/:producto
 * Devuelve el producto y un agregado simple de stock por distrito.
 */
r.get("/ficha-producto/:producto", async (req, res) => {
  ...
  try {
    ...
    const prod = await http
      .get(`${CATALOGO_URL}/productos/${encodeURIComponent(idp)}`, {
        headers: { "X-Correlation-Id": c },
      })
      .then((r) => r.data);
    ...
    if (!prod) {
      return res
        .status(404)
        .json({ code: "NOT_FOUND", message: "Producto no existe" });
    }

    const st = await http
      .get(`${INVENTARIO_URL}/stock`, {
        headers: { "X-Correlation-Id": c },
        params: { id_producto: idp },
      })
      .then((r) => r.data || []);
 ...
 * POST /receta/validar
 * Body: { id_receta }
 * Calcula disponibilidad por sucursal de la receta.
 */
r.post("/receta/validar", async (req, res) => {
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ code: "VALIDATION_ERROR", message: "id_receta requerido" });
    }

    // Importante: RECETAS_URL ya apunta a /recetas
    const receta = await http
      .get(`${RECETAS_URL}/${encodeURIComponent(id)}`, {
        headers: { "X-Correlation-Id": c },
      })
      .then((r) => r.data);
    ...
    const items = await Promise.all(
      (receta.detalle || []).map(async (it) => {
        const st = await http
          ...
          .then((r) => r.data || []);
        const disp = (st || []).reduce(
          (a, s) => a + (s.stock_actual ?? s.cantidad_actual ?? 0),
          0
        );
        return {
          id_producto: it.id_producto,
    ...
    res
      .status(todos ? 200 : 207)
      .json({
        id_receta: id,
        items,
        estado_sugerido: todos
          ? "VALIDADA"
          : items.some((i) => i.disponible > 0)
          ? "PARCIAL"
          : "RECHAZADA",
      });
  } catch (e) {
 ...
 * Idempotente/solo lectura: calcula estado sugerido y sucursal sugerida.
 */
r.get("/receta/:id/validacion", async (req, res) => {
  const c = cid(req);
  try {
    const receta = await http
      .get(`${RECETAS_URL}/${encodeURIComponent(req.params.id)}`, {
        headers: { "X-Correlation-Id": c },
      })
      .then((r) => r.data);
